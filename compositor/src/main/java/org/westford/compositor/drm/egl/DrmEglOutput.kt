...
@AutoFactory(allowSubclasses = true, className = "DrmEglOutputFactory")
class DrmEglOutput internal constructor(@param:Provided private val libc: Libc,
                                        ...
                                        private val eglDisplay: Long) : EglOutput, DrmPageFlipCallback {
    ...
    private var afterPageFlipRender = Optional.empty<Runnable>()
    private var onIdleEventSource = Optional.empty<EventSource>()
    ...
    private fun getFbId(gbmBo: GbmBo): Int {
        ...
        this.libgbm.gbm_bo_set_user_data(gbmBoPtr,
                fb.address,
                Pointerdestroy_user_data.nref(???({ bo, data -> this.destroyUserData(bo, data) })).address)
    ...
    override fun onPageFlip(@Unsigned sequence: Int,
        ...
        this.afterPageFlipRender.ifPresent(Consumer<Runnable> { it.run() })
        this.afterPageFlipRender = Optional.empty<Runnable>()
    }
    ...
    private fun doRender(wlOutput: WlOutput) {
        this.onIdleEventSource = Optional.empty<EventSource>()
        ...
        subscene.geCursorViews()
                .forEach(Consumer<SurfaceView> { painter.paint(it) })
    ...
    private fun toPrimaryPlane(wlOutput: WlOutput,
                               subscene: Subscene): Optional<DrmPlane>? {
        return null
    }

    private fun toPrimaryPlane(wlOutput: WlOutput,
                               view: Optional<SurfaceView>,
                               vararg views: List<SurfaceView>): Optional<DrmPlane>? {
        return null
    }

    private fun toOverlayPlane(wlOutput: WlOutput,
                               views: List<SurfaceView>): Optional<DrmPlane>? {
        return null
    }

    private fun toCursorPlane(wlOutput: WlOutput,
                              surfaceViews: List<SurfaceView>): Optional<DrmPlane>? {
        return null
    }
    ...
    override fun disable() {
        this.afterPageFlipRender = Optional.empty<Runnable>()
        this.onIdleEventSource.ifPresent(Consumer<EventSource> { it.remove() })
        this.enabled = false
    }
    ...
    private fun scheduleRender(wlOutput: WlOutput) {
        ...
        if (this.pageFlipPending) {
            if (!this.afterPageFlipRender.isPresent) {
                this.afterPageFlipRender = Optional.of<Runnable>({ whenIdleDoRender(wlOutput) })
            }
    ...
    private fun whenIdleDoRender(wlOutput: WlOutput) {
        ...
        this.onIdleEventSource = Optional.of(this.display.eventLoop
    ...
    fun setDefaultMode() {
        ...
        val error = this.libdrm.drmModeSetCrtc(this.drmFd,
                ...
                0,
                0,
                Pointer.nref(this.drmOutput.drmModeConnector
    ...
    private fun paintFullscreen(gles2Painter: Gles2Painter,
        ...
        val wlSurface = surfaceView.wlSurfaceResource
                .implementation as WlSurface
        val surface = wlSurface.surface
        val wlBufferResource = surface.state
                .buffer
                .get()
    ...
    private fun getScanoutFormat(clientGbmBo: GbmBo,
        ...
        if (clientFormat == GBM_FORMAT_ARGB8888 && surface.state
                .opaqueRegion
                .isPresent) {
            val opaqueCopy = surface.state
                    .opaqueRegion
                    .get()
                    .copy()
