...
class X11EglPlatformFactory @Inject
internal constructor(private val libxcb: Libxcb,
    ...
    fun create(): X11EglPlatform {
        ...
        val x11EglOutputs = ArrayList<X11EglOutput>(x11Outputs.size)
        val wlOutputs = ArrayList<WlOutput>(x11EglOutputs.size)

        x11Outputs.forEach { x11RenderOutput ->
            x11EglOutputs.add(this.x11EglOutputFactory.create(x11RenderOutput,
                    createEglSurface(eglDisplay,
                            eglConfig,
                            x11RenderOutput.xWindow),
                    eglContext,
                    eglDisplay))
        }
        ...
        this.x11Platform.x11EventBus
                ...
                .connect({ event ->
                    ...
                    when (responseType) {
                        XCB_CLIENT_MESSAGE -> {
                            handle(event.castp(xcb_client_message_event_t::class.java!!),
                                    x11EglPlatform)
                        }
                    }
                })
 ...
 //@formatter:on
    ...
    private fun handle(event: Pointer<xcb_client_message_event_t>,
        ...
        val atom = event.dref()
                ...
                .data32()
                .dref()
        val sourceWindow = event.dref()
                .window()

        if (atom == this.x11Platform.x11Atoms["WM_DELETE_WINDOW"]) {

            val wlOutputIterator = x11EglPlatform.wlOutputs
                    .iterator()
            while (wlOutputIterator.hasNext()) {
                val wlOutput = wlOutputIterator.next()
                val x11EglOutput = wlOutput.output
                        .renderOutput as X11EglOutput
                val x11Output = x11EglOutput.x11Output
                ...
                if (x11Output.xWindow == sourceWindow) {
                    ...
                    x11EglPlatform.renderOutputDestroyedSignal
