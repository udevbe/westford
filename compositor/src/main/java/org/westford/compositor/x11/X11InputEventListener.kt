...
@AutoFactory(className = "X11InputEventListenerFactory", allowSubclasses = true)
class X11InputEventListener internal constructor(@param:Provided private val x11XkbFactory: X11XkbFactory,
                                                 @param:Provided private val x11Platform: X11Platform,
                                                 private val x11Seat: X11Seat) : Slot<Pointer<xcb_generic_event_t>> {

    override fun handle(event: Pointer<xcb_generic_event_t>) {
        val responseType = event.dref()
                .response_type() and 0x7f
        when (responseType) {
            XCB_MOTION_NOTIFY -> {
    ...
    private fun handle(event: xcb_mapping_notify_event_t) {
        if (event.request() === XCB_MAPPING_KEYBOARD) {
            val wlKeyboard = this.x11Seat.wlSeat
                    .wlKeyboard
