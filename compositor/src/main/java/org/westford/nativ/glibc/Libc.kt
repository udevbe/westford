...
class Libc {
    ...
    fun CMSG_SPACE(len: Long): Long {
        return CMSG_ALIGN(len) + CMSG_ALIGN(cmsghdr.SIZE)
    }

    fun CMSG_ALIGN(len: Long): Long {
        return len + Size.sizeof(null as CLong?) - 1 and (Size.sizeof(null as CLong?) - 1).inv().toLong()
    }

    fun CMSG_LEN(len: Long): Long {
        return CMSG_ALIGN(cmsghdr.SIZE + len)
    }

    fun CMSG_DATA(cmsg: Pointer<cmsghdr>): Pointer<Byte> {
        return cmsg.offset(1)
                .castp<Byte>(Byte::class.java!!)
    }

    fun CMSG_FIRSTHDR(mhdr: msghdr): Pointer<cmsghdr> {
        return if (mhdr.msg_controllen()
                .intValue() >= cmsghdr.SIZE)
            mhdr.msg_control()
                    .castp(cmsghdr::class.java)
        else
            Pointer.wrap<cmsghdr>(cmsghdr::class.java!!,
                    0L)
    }
    ...
    @Ptr(String::class)
    external fun strerror(errnum: Int): Long

    var errno: Int
        get() = this.errno_p.dref()
        set(errno) = this.errno_p.write(errno)
    ...
    @Ptr
    external fun mmap(@Ptr addr: Long,
                      len: Int,
                      prot: Int,
                      flags: Int,
                      fildes: Int,
    ...
    @Ptr
    external fun strcpy(@Ptr dest: Long,
                        @Ptr src: Long): Long

    external fun setjmp(@Ptr env: Long): Int

    @Ptr(Void::class)
    external fun memcpy(@Ptr(Void::class) dest: Long,
    ...
    @Unsigned
    fun major(@Unsigned dev: Int): Int {
        return Kdev_t.MAJOR(dev)
    }

    @Unsigned
    fun minor(@Unsigned dev: Int): Int {
        return Kdev_t.MINOR(dev)
    }


    @Ptr(String::class)
    external fun getlogin(): Long
    ...
    @Lng
    external fun send(sockfd: Int,
                      @Ptr buf: Long,
                      @Lng len: Long,
                      flags: Int): Long

    @Lng
    external fun recv(sockfd: Int,
                      @Ptr buf: Long,
                      @Lng len: Long,
                      flags: Int): Long

    @Lng
    external fun recvmsg(sockfd: Int,
                         @Ptr(msghdr::class) msg: Long,
                         flags: Int): Long

    @Lng
    external fun sendmsg(sockfd: Int,
                         @Ptr(msghdr::class) msg: Long,
                         flags: Int): Long

    @Lng
    external fun strlen(@Ptr(String::class) s: Long): Long
