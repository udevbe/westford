...
class LibEGL {
    ...
    @Ptr
    external fun eglCreateContext(@Ptr dpy: Long,
                                  @Ptr config: Long,
                                  @Ptr share_context: Long,
                                  @Ptr attrib_list: Long): Long

    @Ptr
    external fun eglGetDisplay(@Ptr native_display: Long): Long

    @Ptr
    external fun eglCreateWindowSurface(@Ptr display: Long,
                                        @Ptr config: Long,
                                        @Ptr native_window: Long,
                                        @Ptr attrib_list: Long): Long

    @Ptr
    external fun eglQueryString(@Ptr dpy: Long,
                                name: Int): Long
    ...
    @Ptr
    external fun eglGetProcAddress(@Ptr procname: Long): Long
    ...
    fun throwError(failedFunction: String) {
        ...
        when (eglError) {
            ...
            EGL_BAD_MATCH -> {
                throw RuntimeException(failedFunction + " failed - draw or read are not compatible with context, or " +
            ...
            EGL_BAD_NATIVE_PIXMAP -> {
                throw RuntimeException(failedFunction + " failed - a native pixmap underlying either draw or read is " +
                        "no longer valid.")
            }
            EGL_BAD_NATIVE_WINDOW -> {
                throw RuntimeException(failedFunction + " failed - a native window underlying either draw or read is " +
                        "no longer valid.")
            }
            EGL_BAD_CURRENT_SURFACE -> {
                throw RuntimeException(failedFunction + " failed - the previous context has unflushed commands and " +
                        "the previous surface is no longer valid.")
            }
            EGL_BAD_ALLOC -> {
                throw RuntimeException(failedFunction + " failed - allocation of ancillary buffers for draw or read " +
                        "were delayed until eglMakeCurrent is called, and there are not enough " +
                        "resources to allocate them.")
            }
            EGL_CONTEXT_LOST -> {
                throw RuntimeException(failedFunction + " failed - a power management event has occurred. The " +
