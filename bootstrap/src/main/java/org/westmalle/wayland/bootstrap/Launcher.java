...
public class Launcher {
    public static void main(final String[] args) throws IOException, InterruptedException {
        ...
        System.exit(startNewJavaProcess("",
                                        Boot.class.getName(),
                                        args).waitFor());
    }

    private static Process startNewJavaProcess(@Nonnull final String optionsAsString,
                                               @Nonnull final String mainClass,
                                               @Nonnull final String[] arguments) throws IOException {
        Objects.requireNonNull(optionsAsString);
        Objects.requireNonNull(mainClass);
        Objects.requireNonNull(arguments);

        final ProcessBuilder processBuilder = createProcess(optionsAsString,
                                                            mainClass,
                                                            arguments);
    ...
    private static ProcessBuilder createProcess(final String optionsAsString,
        ...
        if (!optionsAsString.isEmpty()) {
            final String[] options = optionsAsString.split(" ");
